...
def simulate_experiment(Cd, angle, v0, tmax, plot=True, tdata=None, xdata=None, vdata=None):
    ...
    xmax = results.y[0,:].max()
    zmax = results.y[1,:].max()
    v_impact = np.sqrt(xmax**2 + zmax**2)

    print('Results')
    print(f'Horizontal distance traveled: {xmax:.2f} m.')
    print(f'Vertical distance traveled: {zmax:.2f} m.')
    print(f'Impact velocity: {v_impact:.2f} m/s')
    print(f'Final time when the cannonball returns to zero elevation {results.t_events[0].item():.2f} s.')
...
res = simulate_experiment(Cd, angle, v0, sim_t)