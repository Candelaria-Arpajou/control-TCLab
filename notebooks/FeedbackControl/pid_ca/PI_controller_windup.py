...
def _(np):
    ...
    SP = np.where(t < 100, 2, 4)

    PV = np.zeros(len(t))
    MV = np.zeros(len(t))
    MV_unlimited = np.zeros(len(t))
    ERROR = np.zeros(len(t))
    return ERROR, K, Ki, Kp, MV, PV, SP, dt, t, tau
...
@app.cell
def _(ERROR, K, Ki, Kp, MV, PV, SP, dataclass, dt, field, np, t, tau):
    @dataclass
    class PIController:
        Kp: float
        Ki: float
        ...
        MV_unlimited:float = field(init=False, default=0.0)
        e_previous: float = field(init=False, default=0.0)

        def update(self, SP, PV):
            e = SP - PV
            ERROR[k] = e
            self.MV += self.Kp * (e - self.e_previous) + self.Ki * self.dt * e
            self.MV = np.clip(self.MV, self.MV_min, self.MV_max)

            self.e_previous = e
            return self.MV

    controller = PIController(Kp=Kp, Ki=Ki, dt=dt, MV_min=0, MV_max=2.5)
