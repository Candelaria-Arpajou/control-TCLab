...
def _(K, Ki, Kp, MV, PV, SP, dataclass, dt, field, np, t, tau):
    ...
    controller = PIController(Kp=Kp, Ki=Ki, dt=dt, MV_min=0, MV_max=100)

    for k in range(1, len(t)):
        MV[k] = controller.update(SP, PV[k-1])
...
def _(MV, PV, SP, np, plt, t):
    ...
    #_ax2.plot(t, ERROR, color="red", linestyle="--", alpha=0.5, label="Error")
    plt.legend()
    plt.show()
    return
